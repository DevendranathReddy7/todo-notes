...
const todoReducer = (state = intialState, action) => {
    switch (action.type) {
        case "ADD_TODO":
            ...
            //     return state
            // } else {
            const withoutError = Array.isArray(state) && state.filter(todo => todo.id !== 0)
            return [...withoutError, {
                id: ++id,
        ...
        case "ERROR_TODO":
            const errorPresnet = Array.isArray(state) && state.some(todo => todo.id === 0 && (todo.todo === action.payload.title))
            const without_Error = Array.isArray(state) && state.filter(todo => todo.id !== 0)
            if (errorPresnet) {
                return state
            } else {
                return [{
